...
const inputSearchBox: HTMLInputElement = <HTMLInputElement>
    document.getElementById("search-box")!; const totalCreditElm: HTMLElement =
    document.getElementById("total-credits")!; renderCoursesInTable(dataCourses);
    renderStudentInTable(infoStudent); totalCreditElm.innerHTML = `${getTotalCredits(dataCourses)}`;
        btnfilterByName.onclick = () => applyFilterByName(); function
        renderCoursesInTable(courses: Course[]): void {
